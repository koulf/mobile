  ...
  Stream<NoticiasState> mapEventToState(
    ...
    if (event is GetNewsEvent) {
      ...
      try {
        ...
        yield NoticiasErrorState(message: "Error al cargar noticias: \n$e");
      }
    }
      ...
      try {
        ...
        news = sportsNews.where((i) {
          for(String x in event.search.split(' '))
            for(String y in i.title.split(' '))
        ...
        news.addAll(businessNews.where((i) {
          for(String x in event.search.split(' '))
            for(String y in i.title.split(' '))
              if(x.toUpperCase() == y.toUpperCase())
                return true;
        ...
        yield FilteredNewsState(
          news: news,
          noticiasSportList: sportsNews,
          noticiasBusinessList: businessNews,        
        );
      } catch (e) {
        yield NoticiasErrorState(message: "Error al cargar noticias por buscar: \n$e");
      }
    }
  }
